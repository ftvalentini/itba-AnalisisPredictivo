...
ibge %>% select(-c(nome_da_grande_regiao,nome_da_unidade_da_federacao)) %>% head()
...
munis_varios = ibge %>% pull(municipio) %>% unique()
...
```{r}
cat(
  setdiff(munis_bolsa, munis_ibge),
  setdiff(munis_bolsa, munis_varios),
...
>**Ejercicios**:
>
>1. obtener cantidad de metropolis (variable `hierarquia`) (a) por estado y (b) por region
>
>2. obtener densidad poblacional (a) por estado y (b) por region
>
>3. obtener la proporción de la población del estado que representa cada municipio
>
>4. obtener los municipios con area_geografica _missing_
...
>**Ejercicios**:
>
>1. Usar Hmisc::cut2() para mejorar efecto visual del color  
>
>2. Hacer un mapa sombreando por alguna variable a nivel estado
>
>3. Hacer mapa de un solo estado sombreando por alguna variable a nivel municipio 
