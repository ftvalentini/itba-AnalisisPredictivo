...
# se agregan variables NO correlacionadas con x1 y x2
x_nocor = replicate(newx*2, runif(n,0,1), simplify=T) %>% as_tibble() %>% 
  setNames(paste0("xnoc_",1:ncol(.)))
dat_exp = list(dat, x_nocor) %>% bind_cols()
...
# modelo con variables irrelevantes
...
Y = d_train$y
l_seq = exp(seq(-2,-6, length.out=50))
cv_lasso = glmnet::cv.glmnet(x=X, y=Y, alpha=1, nfolds=5, lambda=l_seq)
l_opt = cv_lasso$lambda.1se
mod_lasso = glmnet::glmnet(x=X, y=Y, alpha=1, lambda=l_opt)
pred_lasso = predict(mod_lasso, newx=d_test %>% select(-y) %>% as.matrix())
...
mod_lasso %>% broom::tidy() %>% arrange(-abs(estimate))

# MSE
sqrt(sum((pred_col - d_test$y)**2))
sqrt(sum((pred_sim - d_test$y)**2))
sqrt(sum((pred_lasso - d_test$y)**2))
...
gdat = tibble(
  ...
  ,"p = 2" = pred_sim
  ,"p = 802" = pred_col
  ,xlasso = as.vector(pred_lasso)
) %>% 
  tidyr::pivot_longer(-y, names_to="modelo", values_to="fitted")

# plot 1
(
  g_multi_cor = ggplot(gdat %>% dplyr::filter(!modelo %in% "xlasso")
...
ggsave("plots_slides/img/03/multicol_sincorr.png", plot=g_multi_cor
       ,width=6, height=4)
broom::tidy(mod_col) %>% arrange(-abs(estimate))

# plot con lasso
(
  g_multi_sincor_lasso = ggplot(gdat, aes(x=fitted, y=y, color=modelo)) + 
    geom_point() +
    geom_abline() +
    scale_color_brewer(type="qual") +
    NULL
)
ggsave("plots_slides/img/03/multicol_sincorr_lasso.png", plot=g_multi_sincor_lasso
       ,width=6, height=4)
broom::tidy(mod_lasso) %>% arrange(-abs(estimate))

# cv lambda
png("plots_slides/img/03/lambda_cv_lasso.png", width=700, height=300)
plot(cv_lasso)
dev.off()
...
mod_sim %>% broom::tidy() %>% arrange(p.value)
mod_col %>% broom::tidy() %>% arrange(p.value)
